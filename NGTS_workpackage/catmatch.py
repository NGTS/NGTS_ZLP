...
def lmq_fit(best_fit,mycat,cat,RA_lims,DEC_lims,my_X,my_Y,TEL_RA,TEL_DEC,fitlist=['RA_s','DEC_s','CD1_1','CD1_2','CD2_1','CD2_2'],factor=10.0,epsfcn=0.000001,shift=False):
  ...
  x, success = opt.leastsq(lmq_fit_model,priorl,args=(mycat,cat,RA_lims,DEC_lims,my_X,my_Y,pix_coords,TEL_RA,TEL_DEC,name_list,best_fit,shift),factor=factor,epsfcn=epsfcn)

  for i in range(0,len(name_list)):
    best_fit[name_list[i]] = x[i]

  world = load_wcs_from_keywords(best_fit,pix_coords)
