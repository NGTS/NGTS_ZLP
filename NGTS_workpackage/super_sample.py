...
def find_minor_axis(theta, sx, sy):
    ...
    a = ((np.cos(theta)) ** 2.0) / (2 * sx ** 2) + (
        (np.sin(theta)) ** 2.0) / (2 * sy ** 2)

    b = -np.sin(2.0 * theta) / (4 * sx ** 2.0) + sin(2.0 * theta) / (4 * sy **
                                                                     2.0)

    c = ((np.sin(theta)) ** 2.0) / (2 * sx ** 2) + (
        (np.cos(theta)) ** 2.0) / (2 * sy ** 2)

    w_1 = np.sqrt(np.log(2)) / sqrt(a + 2 * b * tan(theta + np.pi / 2.0) + c *
                                    (tan(theta + np.pi / 2.0)) ** 2.0)

    w_2 = np.sqrt(np.log(2)) / sqrt(a + 2 * b * tan(theta) + c *
                                    (tan(theta)) ** 2.0)
...
def call_find_fwhm(file, factor, size, stars, tag=''):
    ...
    for label in labels:
        ...
        fwhm_x_frame, fwhm_y_frame, theta_frame = find_2dfwhm(data[label],
                                                              factor, size)
    ...
    if cache == True:
        for label in labels:
            ...
            pickle.dump(fwhm_x[label], open(tag + 'fwhm_x' + label + '.p',
                                            'wb'))
            pickle.dump(fwhm_y[label], open(tag + 'fwhm_y' + label + '.p',
                                            'wb'))
...
def find_2dfwhm(data, factor, size):
    ...
    x, success = opt.leastsq(
        guassian2d_fit, x1,
        args=(np.arange(1.0 * factor * size) / factor,
              np.arange(1.0 * factor * size) / factor, data))
...
def guassian2d(A, theta, sx, sy, x0, y0, x, y):
    ...
    b = -np.sin(2.0 * theta) / (4 * sx ** 2.0) + np.sin(2.0 * theta) / (4 * sy
                                                                        ** 2.0)

    c = ((np.sin(theta)) ** 2.0) / (2 * sx ** 2) + (
        (np.cos(theta)) ** 2.0) / (2 * sy ** 2)

    f = A * np.exp(-(a * (x0 - xx) ** 2 + 2 * b * (x0 - xx) * (y0 - yy) + c *
                     (y0 - yy) ** 2.0))
    return f
            ...
            IQR = [
                (mean_fluxes <
                 (np.median(mean_fluxes[mean_fluxes >
                                        np.median(mean_fluxes)]))) &
                (mean_fluxes >
                 (np.median(mean_fluxes[mean_fluxes < np.median(mean_fluxes)])))
            ]
...
#      ypos = photdata[2].data['Y_IMAGE'][mean_fluxes > 3*mean(mean_fluxes)]
        ...
        f_5 = [(xpos > mx / 3) & (xpos < 2 * mx / 3) & (ypos < 2 * my / 3) &
               (ypos > mx / 3)]
...
def return_sample_square(c, image, size, factor):
    ...
    r = (size - 1.0) / 2.0
    raw = image[int(round(c[0]) - r - 1):int(round(c[0]) + r),
                int(round(c[1]) - r - 1):int(round(c[1]) + r)]
