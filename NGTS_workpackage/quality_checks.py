...
def get_fwhm(fname, appsize):
    ...
    IQR = [(mean_fluxes <
            (median(mean_fluxes[mean_fluxes > median(mean_fluxes)]))) &
           (mean_fluxes >
            (median(mean_fluxes[mean_fluxes < median(mean_fluxes)])))]
    ...
    cum_profile = array([core1 / total, core / total, core2 / total, core3 /
                         total, core4 / total, core5 / total])
    ...
    x, success = opt.leastsq(cum_guassian_fit, x1,
                             args=(rad, median_cum_profile))
    x[1] = abs(x[1])
